              ...
              <div className="space-y-4">
                ...
                <div className="flex items-center justify-between mt-6">
                  ...
                  <button
                    onClick={() =>
                      setCurrentPage((prev) =>
                        Math.min(prev + 1, totalTicketPages)
                      )
                    }
            ...
            <div className="space-y-4">
              ...
              <div className="flex gap-4 mb-6">
                ...
                <select
                  ...
                  onChange={(e) => setDrawResultFilter(e.target.value)}
                >
                  <option value="all">All Lotteries</option>
                  <option value="lotto g 100">Lotto G 100</option>
                  <option value="lotto g 200">Lotto G 200</option>
                  <option value="lotto g 300">Lotto G 300</option>
                  <option value="lotto g 500">Lotto G 500</option>
                </select>
              </div>
              ...
              <div className="bg-white rounded-lg shadow">
                ...
                {currentDrawResults.map((result) => (
                  <div
                    className="p-4 border-b cursor-pointer hover:bg-gray-50"
                    key={result.id}
                    onClick={() => router.push(`/draw/details?id=${result.id}`)}
                  >
                    <div className="space-y-2">
                      <div className="flex justify-between">
                        <h3 className="font-semibold">{result.lotteryName}</h3>
                        <p className="text-gray-500">
                          {format(
                            parse(result.drawDate, "d-M-yyyy", new Date()),
                            "dd-MM-yyyy"
                          )}
                        </p>
                      </div>
                      <div className="flex gap-2">
                        {result.winningNumbers.slice(0, 5).map((num) => (
                          <span
                            key={num}
                      ...
                      <div className="mt-4">
                        <h4 className="font-medium">Prize Tiers</h4>
                        <div className="mt-2 space-y-2">
                          {result.prizeTiers.slice(0, 2).map((prize) => (
                            <div
                            ...
                            >
                              ...
                              <span>
                                ₹{prize.amount} ({prize.winners.length} winners)
                              </span>
                            </div>
                          ))}
                        </div>
              ...
              <div className="flex items-center justify-between mt-6">
                ...
                <button
                  onClick={() =>
                    setCurrentPage((prev) =>
                      Math.min(prev + 1, totalDrawResultPages)
                    )
                  }
                  disabled={currentPage === totalDrawResultPages}
                  className="px-4 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-md disabled:opacity-50"
                >
                  Next
                </button>
