...
const StoreCustomization: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setMessage("Failed to update store customization. Please try again.");
      console.log("Failed to update store customization", error);
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleLogoUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      const response = await axios.post("/api/retailer/upload-logo", formData);
      setFormData((prev) => ({
        ...prev,
